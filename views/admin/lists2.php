...
<?php require('nav.php');?>
    ...
    <ol class="breadcrumb">
        <li><a data-toggle="modal" data-target="#addType">添加小类别</a></li>
    </ol>
    ...
    <div class="table-responsive">
        <table class="table table-striped table-hover">
            ...
            <tr>
                ...
                <th><span class="glyphicon glyphicon-user"></span> <span class="visible-lg">父名称</span></th>
...
<div class="modal fade" id="addType" tabindex="-1" role="dialog" aria-labelledby="addUserModalLabel">
    <div class="modal-dialog" role="document" style="max-width:450px;">
        <form action="/type/add2" method="post" autocomplete="off" draggable="false">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title" >增加小分类</h4>
                </div>
            ...
            if (name.attr("name") === "see") {
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        $('#sort').val(data.sort);
                        $('#listsid').val(data.id);
                        $('#category-fname ').find("option[value='"+data.pid+"']").attr('selected','selected');
                        $('#seeUser').modal('show');
                    }
                });
                ...
                if (window.confirm("此操作不可逆，是否确认？")) {
                    $.ajax({
                        type: "type",
                        url: "/type/delete",
                        data: {"id":id,"_csrf":"<?= Yii::$app->request->csrfToken ?>",'type':1},
                        cache: false, //不缓存此页面
                        success: function (data) {
